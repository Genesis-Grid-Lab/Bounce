...
namespace Engine {
  ...
  Animation::Animation(const std::string& animationPath, Ref<Model> model):m_AssimpAnimation(nullptr), m_Model(model),m_Path(animationPath){
    ...
    if(scene == nullptr){
      E_CORE_ERROR("Failed to load {}", animationPath);
    }        
    ...
    for (int i = 0; i < m_AssimpAnimation->mNumChannels; i++) {
      ...
      E_CORE_INFO("Animation Channel: {}", channel->mNodeName.C_Str());
    }  
              
    m_Duration = m_AssimpAnimation->mDuration;
    m_TicksPerSecond = m_AssimpAnimation->mTicksPerSecond;
    E_CORE_INFO("Loaded animation: {} | Duration: {} | TicksPerSecond: {}", animationPath, m_Duration, m_TicksPerSecond);
    ...
    if (m_Model) {
      ...
      E_CORE_WARN("Animation constructed without model. Call SetModel(model) before playing.");
    }
  }

  void Animation::SetModel(Ref<Model> model) {
    if (!m_AssimpAnimation || m_Bones.size() > 0) {
      E_CORE_ERROR("Cannot bind model: No animation loaded.");
      return;
    }
    if (!model) {
      E_CORE_ERROR("Cannot bind null model.");
      return;
    }
