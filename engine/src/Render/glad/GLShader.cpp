 ...
 static GLenum ShaderTypeFromString(const std::string& type)
{
    ...
    E_CORE_ASSERT(false, "Unknown shader type!");
    return 0;
}
...
std::string GLShader::ReadFile(const std::string& filepath)
{
    ...
    if (in)
    {
        ...
        {
            E_CORE_ERROR("Could not read from file '{0}'", filepath);
        }
    }
    else
    {
        E_CORE_ERROR("Could not open file '{0}'", filepath);
    }
...
std::unordered_map<GLenum, std::string> GLShader::PreProcess(const std::string& source)
{
    ...
    while (pos != std::string::npos)
    {
        ...
        E_CORE_ASSERT(eol != std::string::npos, "Syntax error");
        size_t begin = pos + typeTokenLength + 1; //Start of shader type name (after "#type " keyword)
        std::string type = source.substr(begin, eol - begin);
        E_CORE_ASSERT(ShaderTypeFromString(type), "Invalid shader type specified");

        size_t nextLinePos = source.find_first_not_of("\r\n", eol); //Start of shader code after shader type declaration line
        E_CORE_ASSERT(nextLinePos != std::string::npos, "Syntax error");
...
void GLShader::Compile(const std::unordered_map<GLenum, std::string>& shaderSources)
{
    ...
    E_CORE_ASSERT(shaderSources.size() <= 2, "We only support 2 shaders for now");
    ...
    for (auto& kv : shaderSources)
    {
        ...
        if (isCompiled == GL_FALSE)
        {
            ...
            E_CORE_ASSERT(false, "Shader compilation failure!");
            break;
        }
    ...
    if (isLinked == GL_FALSE)
    {
        ...
        E_CORE_ERROR("{0}", infoLog.data());
        E_CORE_ASSERT(false, "Shader link failure!");
        return;
    }
