...
static GLenum ShaderTypeFromString(const std::string& type)
{
  ...
  E_CORE_ERROR("Unknown shader type '{}'", type);
  return 0;
}
...
static bool compile(GLuint sh, const char* src, std::string* log){
  ...
  if (!ok) {
    ...
    if (log)
      *log += message;
    E_CORE_ERROR("Shader compilation failed:\n{}", message);
  }
  ...
  while (pos != std::string::npos)
    {
      ...
      if (eol == std::string::npos)
        {
          E_CORE_ERROR("Shader preprocessing error: missing end of line after '#type' declaration.");
          break;
        }
      ...
      if (begin == std::string::npos || begin >= eol)
        {
          E_CORE_ERROR("Shader preprocessing error: missing shader type after '#type'.");
      ...
      if (nextLinePos == std::string::npos)
        {
          E_CORE_ERROR("Shader preprocessing error: missing shader code for type '{}'", type);
          break;
        }
      ...
      if (shaderType == 0)
        {
          E_CORE_ERROR("Shader preprocessing error: invalid shader type '{}'", type);
          continue;
        }
...
bool GLShader::CompileFromSource(const char *vs, const char *fs,
  ...
  if(!ok){ GLint len=0; glGetProgramiv(p,GL_INFO_LOG_LENGTH,&len);
    ...
    if (outLog)
      *outLog = linkLog;
    E_CORE_ERROR("Shader linking failed:\n{}", linkLog);
    glDeleteProgram(p);
    return false;
  }
  m_Program = p;
  ...
  if (in)
    {
      ...
      if (size != -1)
        ...
        {
          E_CORE_ERROR("Could not read from file '{}'", filepath);
        }
    }
  else
    {
      E_CORE_ERROR("Could not open file '{}'", filepath);
    }
