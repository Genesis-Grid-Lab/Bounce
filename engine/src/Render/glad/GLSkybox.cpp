...
namespace Engine {
    ...
    GLSkybox::GLSkybox(const std::vector<std::string>& faces)
    {
    ...
    void GLSkybox::Draw(const Ref<Shader>& shader, const glm::mat4& view, const glm::mat4& projection)
    {
    ...
    Ref<Mesh> GLSkybox::CreateCubeMesh()
    {
    ...
    uint32_t GLSkybox::LoadCubemap(const std::vector<std::string>& faces)
    {
        uint32_t textureID;
        ...
        for (uint32_t i = 0; i < faces.size(); ++i) {
            ...
            if (data) {
                ...
                E_CORE_WARN("Cubemap texture failed to load at path: {0}", faces[i]);
                stbi_image_free(data);
            }
        }
