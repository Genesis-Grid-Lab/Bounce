...
#include "glad/glad.h"
#include "Core/Log.h"
#include "Render/glad/GLTexture2D.h"
...
bool GLTexture2D::Create(uint32_t w, uint32_t h, int channels, const void* pixels){
    if(channels!=3 && channels!=4) {
        E_CORE_ERROR("GLTexture2D invalid channel count: {}", channels);
        return false;
    }
...
bool GLTexture2D::LoadFromFile(const char* path, bool flipY){
    ...
    if(!data){ E_CORE_ERROR("GLTexture2D failed to load {}", path); return false; }
    if(n!=3 && n!=4) { // convert to RGBA
        unsigned char* rgba = (unsigned char*)malloc((size_t)w*h*4);
        if(!rgba){
            E_CORE_ERROR("GLTexture2D RGBA allocation failed for {}", path);
