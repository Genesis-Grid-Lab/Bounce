...
#include <stb_image.h>
#include "Auxiliaries/AssimpToGlm.h"
...
namespace Engine {
  ...
  void Model::LoadModel(const std::string& path){
    ...
    if (!scene || scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE || !scene->mRootNode) {            
      E_CORE_ERROR("Assimp Error: {0}", importer.GetErrorString());
      return;
    }
  ...
  Ref<Mesh> Model::ProcessMesh(aiMesh* mesh, const aiScene* scene){
    ...
    E_CORE_WARN("Mesh with {0} vertices and {1} indices", vertices.size(), indices.size());
  ...
  std::vector<TextureMesh> Model::LoadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName){      
	...
	E_CORE_INFO("Loaded tex type {0} path {1}", texture.type, str.C_Str());
	// if(texture.IsLoaded()){
	//     textures.push_back(texture);
	//     m_TexturesLoaded.push_back(texture);
