...
static std::vector<char> readFile(const std::string& filename) {
    ...
    if (!file.is_open()) {
        E_CORE_ASSERT(false,"failed to open file!");
    }
...
namespace Engine {
  ...
  bool VulkanDevice::Initialize(Window &window) {
    m_Window = &window;
    E_CORE_INFO("[Vulkan] Init");
    createInstance();
    setupDebugMessenger();
    createSurface();
    pickPhysicalDevice();
  ...
  void VulkanDevice::pickPhysicalDevice() {
    ...
    if (deviceCount == 0) {
      E_CORE_ASSERT(false, "failed to find GPUs with Vulkan support!");
    }
    ...
    if (physicalDevice == VK_NULL_HANDLE) {
      E_CORE_ASSERT(false,"failed to find a suitable GPU!");
    }
  ...
  void VulkanDevice::createLogicalDevice(){
    ...
    if (vkCreateDevice(physicalDevice, &createInfo, nullptr, &device) != VK_SUCCESS) {
      E_CORE_ASSERT(false,"failed to create logical device!");
    }
  ...
  void VulkanDevice::createSurface(){
    if (glfwCreateWindowSurface(instance, (GLFWwindow*)m_Window->Handle(), nullptr, &surface) != VK_SUCCESS) {
      E_CORE_ASSERT(false,"failed to create window surface!");
    }        
  }
  ...
  void VulkanDevice::createSwapChain(){
    ...
    if (vkCreateSwapchainKHR(device, &createInfo, nullptr, &swapChain) != VK_SUCCESS) {
      E_CORE_ASSERT(false,"failed to create swap chain!");
    }
  ...
  void VulkanDevice::createImageViews(){
	...
	E_CORE_ASSERT(false,"failed to create image views!");
      }
    }
  }
  ...
  void VulkanDevice::createRenderPass() {
    ...
    if (vkCreateRenderPass(device, &renderPassInfo, nullptr, &renderPass) != VK_SUCCESS) {
      E_CORE_ASSERT(false,"failed to create render pass!");
    }
  }
  ...
  void VulkanDevice::createGraphicsPipeline(){
    ...
    if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &pipelineLayout) != VK_SUCCESS) {
      E_CORE_ASSERT(false,"failed to create pipeline layout!");
    }
    ...
    if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) != VK_SUCCESS) {
      E_CORE_ASSERT(false,"failed to create graphics pipeline!");
    }
  ...
  void VulkanDevice::createFramebuffers() {
    ...
    for (size_t i = 0; i < swapChainImageViews.size(); i++) {
      ...
      if (vkCreateFramebuffer(device, &framebufferInfo, nullptr, &swapChainFramebuffers[i]) != VK_SUCCESS) {
        E_CORE_ASSERT(false,"failed to create framebuffer!");
      }
    }
  }
  ...
  void VulkanDevice::createCommandPool() {
    ...
    if (vkCreateCommandPool(device, &poolInfo, nullptr, &commandPool) != VK_SUCCESS) {
      E_CORE_ASSERT(false,"failed to create command pool!");
    }
  }
  ...
  void VulkanDevice::createCommandBuffer() {
    ...
    if (vkAllocateCommandBuffers(device, &allocInfo, &commandBuffer) != VK_SUCCESS) {
      E_CORE_ASSERT(false,"failed to allocate command buffers!");
    }
  }
  ...
  void VulkanDevice::createSyncObjects() {
	...
	vkCreateFence(device, &fenceInfo, nullptr, &inFlightFence) != VK_SUCCESS) {
      E_CORE_ASSERT(false,"failed to create semaphores!");
    }
  }
  ...
  void VulkanDevice::drawFrame(){
    ...
    if (vkQueueSubmit(graphicsQueue, 1, &submitInfo, inFlightFence) != VK_SUCCESS) {
      E_CORE_ASSERT(false,"failed to submit draw command buffer!");
    }
  ...
  VkExtent2D VulkanDevice::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities){
    if (capabilities.currentExtent.width != std::numeric_limits<uint32_t>::max()) {
      ...
      int width, height;
      glfwGetFramebufferSize((GLFWwindow*)m_Window->Handle(), &width, &height);
  ...
  VkShaderModule VulkanDevice::createShaderModule(const std::vector<char>& code){
    ...
    if (vkCreateShaderModule(device, &createInfo, nullptr, &shaderModule) != VK_SUCCESS) {
      E_CORE_ASSERT(false,"failed to create shader module!");
    }
  ...
  void VulkanDevice::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex){
    ...
    if (vkBeginCommandBuffer(commandBuffer, &beginInfo) != VK_SUCCESS) {
      E_CORE_ASSERT(false,"failed to begin recording command buffer!");
    }
    ...
    if (vkEndCommandBuffer(commandBuffer) != VK_SUCCESS) {
      E_CORE_ASSERT(false,"failed to record command buffer!");
    }
  }
}
